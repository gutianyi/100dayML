...
dataset = pd.read_csv(
    "raw_price_train/2_r_price_train.csv",
    index_col='Date')
...
for epoch in range(3001):  # 训练批次数
    ...
    if test_mse < best_test_mse:
        ...
        torch.save(model.state_dict(), 'stock-cnn-adj_vol-model.pt')
...
model.load_state_dict(torch.load('stock-cnn-adj_vol-model.pt'))
pred = model(torch.Tensor(X).view(-1, m, 2)).detach().numpy()
# pred = model(torch.Tensor(final_pred).view(1, m, 2)).detach().numpy()
print(pred)
...
plt.title('CNN-Adj Close_Vol-Result')
...
plt.savefig('CNN-Adj Close_Vol-Result.jpg')
plt.show()
