...
X,y = feature_engineering_hmm_RNN(data,n_states,m,n)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input shape ( None, 14, n_states+1)
            nn.Conv1d(
                in_channels= m,  # input height
            ...
            nn.ReLU(),  # activation
            nn.MaxPool1d(kernel_size=2),  # 在 2x2 空间里向下采样, output shape (batch_size, 16, (n_states+1)/ 2)
        )
...
def train(model, loader, optimizer, loss_func):
    ...
    for step, (batch_x, batch_y) in enumerate(loader):  # 每一步 loader 释放一小批数据用来学习
        # 训练的地方...
        batch_x = batch_x.view(-1, m, n_states+1)  # reshape x to (batch, time_step, input_size)
...
model.load_state_dict(torch.load('stock-cnn-Adj_Hmm-model.pt'))
pred = model(torch.Tensor(X).view(-1, m, n_states+1)).detach().numpy()
print(pred.shape)
...
plt.title('CNN-Adj Close_Hmm-Result')
...
plt.savefig('CNN-Adj Close_Hmm-Result.jpg')
plt.show()

print(model(torch.Tensor().view(-1, m, n_states+1)).detach().numpy())
#
